...
def get_typingMatrices(df: pd.DataFrame):
    ...
    df['hours_since_start'] = df['keypressTimestampLocal'].dt.hour + (df['keypressTimestampLocal'].dt.minute / 60)
    df['hour_blocks'] = np.where((df['hours_since_start'] % 1) > 0.5, df['hours_since_start'].astype(int)+0.5, df['hours_since_start'].astype(int))
    dictHourBlocks = dict(zip(np.arange(0, 24, 0.5),list(range(len(np.arange(0, 24, 0.5))))))
    df['hourBlockIdx'] = df['hour_blocks'].map(dictHourBlocks)
    
    M = df.groupby(['dayNumber','hourBlockIdx'],as_index = False).size().pivot(index='dayNumber', columns='hourBlockIdx').fillna(0)

    # insert hours with no activity across all days
    missingHours = [h for h in range(48) if h not in list(M['size'].columns)]
    ...
    speedM=df.groupby(['dayNumber','hourBlockIdx'],as_index = False).apply(lambda x: medianAAIKD(x)).pivot(index='dayNumber', columns='hourBlockIdx')
...
def get_sleepWakeLabels(svd_mat):
    ...
    for r in range(svd_mat.shape[0]):
        ...
        if ((binarizedSVD[r, 4:30] == np.array([wake_label]*26)).all()) == True:
            idx_rowMin = np.argmin(svd_mat[r])
        else: # else limit min hour to between hr 2-15
            idx_rowMin = np.argmin(svd_mat[r, 4:30]) + 4
        ...
        if r == svd_mat.shape[0]-1:
            ...
            if (row[-1] == sleep_label):
                ...
                while row[47-i] == sleep_label:
                    end_idx = i
                    i += 1
                # replace identified ending cells with sleep label
                floodFillM[r,(47-end_idx):] = sleep_label
...
def plot_heatmaps(activityM, speedM, svdM, sleepWakeMatrix):
    ...
    ax[0,0].set(title='Input Typing Activity', xlabel='Hour Index', ylabel='Day')
    ax[0,1].set(title='Input Typing Speed', xlabel='Hour Index', ylabel='Day')
    ax[1,0].set(title='Graph Regularized SVD', xlabel='Hour Index', ylabel='Day')    
    ax[1,1].set(title='Sleep/Wake Labels', xlabel='Hour Index', ylabel='Day')
    f.tight_layout()
    return f
...
if __name__ == '__main__':
    ...
    pathFig = '/home/mindy/Desktop/BiAffect-iOS/UnMASCK/graph_regularized_SVD/figures/blocks30min/'
