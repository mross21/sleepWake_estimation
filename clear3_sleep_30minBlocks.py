...
from save_gsvds_30minBlocks import save_gsvds_30minBlocks
import pandas as pd
import numpy as np
...
def reshape_sleep_labels(sleepM):
    ...
    addNaN1 = np.insert(flatM, 0, [np.nan]*(47-idx))
...
gsvd_file = 'gsvd_results_30minBlocks.pkl'

# Calculate graph SVDs, save them to file, and read them back in.
save_gsvds_30minBlocks(dat_dir, all_files, subs, out_file=gsvd_file)
...
for sub, res in gsvd_results.items():
    ...
    avgActivityPerDay = Mbinary.mean(axis=1).mean() / 2
    # of the days with kp, find median amount
    Mkp = np.where(Mactivity > 0, Mactivity, np.nan)
    avgAmountPerDay = np.nanmedian(np.nanmedian(Mkp, axis=1)) / 2
    ...
    plot.savefig(dat_dir+"images/matrices_sub-{}_30minBlocks.png".format(sub), dpi=300)
    plt.close()
    ...
    sleep_pred = np.sum(1 - reshapedSleepMatrix, axis=1) / 2
    ...
    plt.savefig(dat_dir+"images/scatter_sub-{}_30minBlocks.png".format(sub), dpi=300)
    plt.close()
    ...
    plt.savefig(dat_dir+"images/duration_sub-{}_30minBlocks.png".format(sub), dpi=300)
    plt.close()
    ...
    sns.heatmap(svd, cmap='viridis', ax=ax3,
                cbar_kws={'label': 'Typing intensity', 'fraction': 0.043})
    plt.setp(ax3.get_yticklabels(), visible=False)
    plt.savefig(dat_dir+"images/quality_sub-{}_30minBlocks.png".format(sub), dpi=300)
    plt.close()
...
plt.savefig(dat_dir+"images/cor_days_self-report_30minBlocks.png", dpi=300)
...
plt.savefig(dat_dir+"images/cor_activity_30minBlocks.png", dpi=300)
...
plt.savefig(dat_dir+"images/cor_days_mat_30minBlocks.png", dpi=300)
...
plt.savefig(dat_dir+"images/cor_activity_per_day_30minBlocks.png", dpi=300)
